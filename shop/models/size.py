...
class Size(BaseModel):
    size = models.IntegerField(
        default=200, blank=True, verbose_name=_("Size")
    )  # 신발 사이즈
    size_code = models.CharField(
        max_length=3,
        blank=True,
        choices=SizeCodeChoices.choices,
        verbose_name=_("Size Code"),
    )  # XS,S,M,L,XL,2XL
    outseam = models.IntegerField(
        default=0, blank=True, verbose_name=_("Outseam")
    )  # 바지총장
    shoulder_width = models.IntegerField(
        default=0, blank=True, verbose_name=_("Shoulder Width")
    )  # 어깨 너비
    chest_width = models.IntegerField(
        default=0, blank=True, verbose_name=_("Chest Width")
    )  # 가슴 너비
    sleeve_length = models.IntegerField(
        default=0, blank=True, verbose_name=_("Sleeve Length")
    )  # 소매 길이
    waist_width = models.IntegerField(
        default=0, blank=True, verbose_name=_("Waist Width")
    )  # 허리 너비
    thigh_width = models.IntegerField(
        default=0, blank=True, verbose_name=_("Thigh Width")
    )  # 허벅지 너비
    ...
    clothes_division = models.CharField(
        ...
        default=DivisionChoices.TOP,
        verbose_name=_("Clothes Division"),
